        ...
        <table class="w-full text-left border-collapse whitespace-nowrap lg:whitespace-normal">
            ...
            <tbody class="text-[13px] text-slate-600 divide-y divide-slate-100">
                ...
                <tr class="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td class="py-3.5 px-6">
                        <form action="{{ route('admin.pesanan.status') }}" method="POST" class="inline-block">
                            ...
                            <select name="status" onchange="this.form.submit()" class="text-xs font-semibold py-1.5 px-2.5 rounded-full outline-none cursor-pointer border
                                {{ $o->status == 'Sedang Disiapkan' ? 'bg-amber-50 text-amber-600 border-amber-200' : '' }}
                                {{ $o->status == 'Selesai' ? 'bg-green-50 text-green-600 border-green-200' : '' }}
                                {{ $o->status == 'Dikirim' ? 'bg-blue-50 text-blue-600 border-blue-200' : '' }}
                                {{ $o->status == 'Dibatalkan' ? 'bg-red-50 text-red-600 border-red-200' : '' }}
                            ">
                                <option value="Sedang Disiapkan" {{ $o->status == 'Sedang Disiapkan' ? 'selected' : '' }}>Sedang Disiapkan</option>
                    ...
                    <td class="py-3.5 px-6">
                        <button class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-600 hover:text-white transition-colors" onclick="alert('Alamat: {{ $o->alamat }}\nWaktu: {{ $o->created_at->format('d/m/Y H:i') }}')">
                            Detail
                        </button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="text-center py-10 text-slate-400">Belum ada data pesanan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
